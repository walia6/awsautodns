...
a =
    json.decode(
    execute(
        'aws ec2 describe-tags --filters "Name=resource-id,Values=' .. instanceid .. '" "Name=key,Values=autodnsdomain"'
    )
).Tags
if #a == 0 then
...
a =
    ...
    execute(
        'aws ec2 describe-tags --filters "Name=resource-id,Values=' ..
            instanceid .. '" "Name=key,Values=autodnshostedzoneid"'
    )
).Tags
if #a == 0 then
...
a =
    ...
    execute(
        "aws route53 change-resource-record-sets --hosted-zone-id " ..
            hostedzoneid .. " --change-batch file:///opt/awsautodns/changes.json"
    )
)
log(
    "Updated the DNS record of hosted zone ID " ..
        hostedzoneid .. " to point " .. domain .. " to " .. publicip .. ". Change ID: " .. a.ChangeInfo.Id .. "."
)
